...
{
    assert(p);
    p->recent = std::chrono::steady_clock::now();
}
...
int ElapsedTimer::elapsed()
{
    ...
    return static_cast<int>(duration_cast<milliseconds>(std::chrono::steady_clock::now() - p->recent).count());
}
...
int64_t ElapsedTimer::nanoelapsed()
{
    ...
    using std::chrono::duration_cast;
    return duration_cast<nanoseconds>(std::chrono::steady_clock::now() - p->recent).count();
}
