...
std::string     LogWriter::m_logDirPath = "kokoko";
std::string     LogWriter::m_logFilePath;
std::string     LogWriter::m_logPrefix;
PGE_LogLevel    LogWriter::m_logLevel;
int             LogWriter::m_maxFilesCount = 10;
int64_t         LogWriter::m_bytesWritten = 0;
int64_t         LogWriter::m_bytesWrittenMax = 2097152;
bool            LogWriter::m_enabled = false;
bool            LogWriter::m_singleFile = false;
bool            LogWriter::m_enabledStdOut = true;
bool            LogWriter::m_enabledVerboseLogs = true;

std::mutex      LogWriter::mtx;

bool  LogWriter::m_logIsOpened = false;
FILE *LogWriter::m_logout = nullptr;

void LogSetSingleFileMode()
{
    LogWriter::m_singleFile = true;
}
...
static std::string dateTime()
{
    ...
    char out[24];
    std::memset(out, 0, sizeof(out));
    if(0 < strftime(out, sizeof(out), "%Y-%m-%d %H:%M:%S", std::localtime(&in_time_t)))
        return std::string(out);
    else
        return "0000-00-00 00:00:00";
}

static void cleanUpLogs(const std::string &prefix, const std::string &logsPath, int maxLogs)
{
    if(maxLogs <= 0)
        return; // No limit
...
void LogWriter::LoadLogSettings(const std::string &logPath, const std::string &prefix, bool disableStdOut, bool verboseLogs)
{
...
#else
    m_logPrefix = prefix;
    m_logDirPath = logPath;

    std::string logFileName = prefix + "_log_" + return_current_time_and_date() + ".txt";

    if(m_singleFile)
        logFileName = prefix + "_log.txt";
...
//    logSettings.endGroup();
    ...
    if(!disableStdOut)
        std::printf("LogLevel %d, log file %s\n\n", static_cast<int>(m_logLevel), m_logFilePath.c_str());

    reopenLog();
#endif
}

void LogWriter::reopenLog()
{
    long lastLogSize = 0;

    if(m_logout) // Close previously opened log
    {
        fseek(m_logout, 0L, SEEK_END);
        lastLogSize = ftell(m_logout);
        fclose(m_logout);
    ...
    {
        std::lock_guard<std::mutex> lockMtx(LogWriter::mtx);

        m_logout = fopen(m_logFilePath.c_str(), "a");
        if(m_logout)
        {
            fseek(m_logout, 0L, SEEK_END);
            lastLogSize = ftell(m_logout);
            ...
            LogWriter::m_bytesWritten = lastLogSize;
        }
    }

    std::string logFileName = m_logPrefix + "_log_" + return_current_time_and_date() + ".txt";

    if(m_singleFile)
        logFileName = m_logPrefix + "_log.txt";
    ...
    cleanUpLogs(m_logPrefix, m_logDirPath, m_maxFilesCount);

    if(m_enabled)
    {
        if(lastLogSize > m_bytesWrittenMax) {
            m_logout = fopen(m_logFilePath.c_str(), "w");
            m_bytesWritten = 0;
        }
        else {
            m_logout = fopen(m_logFilePath.c_str(), "a");
            LogWriter::m_bytesWritten = ftell(m_logout);
        }
        if(m_logout)
        {
            m_logIsOpened = true;
...
void LoadLogSettings(const std::string &logPath, const std::string &prefix, bool disableStdOut, bool verboseLogs)
{
    LogWriter::LoadLogSettings(logPath, prefix, disableStdOut, verboseLogs);
}
...
#else
    va_copy(arg_in, arg);
    int len1 = std::snprintf(g_outputBuffer, OUT_BUFFER_SIZE, "%s [%s]: ", label, dt.c_str());
    std::fwrite( g_outputBuffer, 1, (size_t)len1, LogWriter::m_logout);
    int len2 = std::vsnprintf(g_outputBuffer, OUT_BUFFER_SIZE, format, arg_in);
    std::fwrite(g_outputBuffer, 1, (size_t)len2, LogWriter::m_logout);
    std::fwrite(reinterpret_cast<const void *>(OS_NEWLINE), 1, OS_NEWLINE_LEN, LogWriter::m_logout);
    va_end(arg_in);
    std::fflush(LogWriter::m_logout);

    if(len1 > 0)
    {
        std::lock_guard<std::mutex> lockMtx(LogWriter::mtx);
        LogWriter::m_bytesWritten += len1;
    }

    if(len2 > 0)
    {
        std::lock_guard<std::mutex> lockMtx(LogWriter::mtx);
        LogWriter::m_bytesWritten += len2;
    }

    if(LogWriter::m_bytesWritten >= LogWriter::m_bytesWrittenMax)
       LogWriter::reopenLog();
#endif
